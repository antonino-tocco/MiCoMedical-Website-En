<?php
	...
	define('THEME_CSS_DIR', THEME_URI . '/dist/css');
	define('THEME_JS_DIR', THEME_URI . '/dist/js');
	define('THEME_CSS_URI', THEME_URI . '/dist/css');
						...
						switch ($post_format) {
							...
							default:
								if ( $thumbnail ){
									...
									echo "<div class='pic'><img src='". ($post_img ? $post_img : bfi_thumb($thumbnail,$thumbnail_dims)) ."' alt />". ($post_img ? "</div>" : "<div class='hover-effect'></div><div class='links'>$emb_video</div></div>");
									$some_media = true;
								} break;
						}
					?>
	...
	function cws_post_content_output ($chars_count=0, $blogtype=""){
		...
		if ($post_type == "post"){
			...
			if ( $blogtype == "pinterest" ){
				$content =  !empty( $post->post_excerpt ) ? get_the_excerpt() :  substr( preg_replace( "/\s{2,}/u", " ", strip_tags( strip_shortcodes( get_the_content("") ) ) ), 0, $chars_count );
			}
			else{
				$content =  !empty( $post->post_excerpt ) ? get_the_excerpt() :  cws_closetags( substr( preg_replace( "/\s{2,}/u", " ", strip_shortcodes( get_the_content("") ) ), 0, $chars_count ) );
			}
		}
		else if ($post_type == "portfolio"){
			$content = preg_replace( "/\s{2,}/u", " ", strip_tags(strip_shortcodes(get_the_content(""))) );
		...
		$more = 1;
		$full_content = preg_replace( "/\s{2,}/u", " ", strip_tags(strip_shortcodes(get_the_content(""))) );
...
add_filter( 'mce_buttons_2', 'clinico_mce_buttons_2' );

function clinico_mce_buttons_2( $buttons ) {
	array_unshift( $buttons, 'styleselect' );
	return $buttons;
}

add_filter( 'tiny_mce_before_init', 'clinico_tiny_mce_before_init' );

function clinico_tiny_mce_before_init( $settings ) {

	$settings['theme_advanced_blockformats'] = 'p,h1,h2,h3,h4';

	$style_formats = array(
	array( 'title' => 'Clinico title', 'block' => 'div', 'classes' => 'widget-title'),
...
add_filter('redux/font-icons' , 'get_all_fa_icons');
