...
<html class="not-ie no-js" xmlns="http://www.w3.org/1999/xhtml" <?php language_attributes(); ?>>
	...
	?>
		...
		<link rel="stylesheet" type="text/css" href="<?php echo get_template_directory_uri();?>/bower_components/jquery-jvectormap-2.0.3/jquery-jvectormap-2.0.3.css"/>
		<link rel="stylesheet" type="text/css" href="<?php echo get_template_directory_uri();?>/bower_components/bootstrap-switch/dist/css/bootstrap3/bootstrap-switch.min.css"/>
	...
	<body <?php body_class(); ?>
		ng-app="ClinicoApp">
		<div id="main-container" ng-controller="BootController">
		<?php
			if ($post) {
		...
		<?php
			...
			?>
				<div class="container">
					...
					<?php
						if (function_exists("DISPLAY_ACURAX_ICONS")) { DISPLAY_ACURAX_ICONS(); }
						cws_show_logo();
						echo '<div class="logo partner-logo" style="float:right"><img style="max-width:250px" src="' . get_template_directory_uri().'/img/logo-nk-2016.jpg" class="img-responsive" /></div>';
						//<!--Start main menu-->
						global $current_user;
						...
						echo '<ul class="nav navbar-nav navbar-right lang-menu">';
						echo '<li class="dropdown menu-item">';
						echo '<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">'
							.'<img src="'. get_template_directory_uri(). '/img/flags-mini/gb.png"> EN <span class="caret"></span></a>';
						echo '<ul class="dropdown-menu sub-menu">';
						echo '<li class="menu-item"><a href="#"><img src="'. get_template_directory_uri(). '/img/flags-mini/gb.png"> EN</a></li>';
						echo '<li class="menu-item"><a href="'. get_field("url_italiano") .'"><img src="'. get_template_directory_uri(). '/img/flags-mini/it.png"> IT</a></li>';
						echo '</ul>';
						echo '</li>';
						echo '</ul>';
						echo '</nav>';
				...
				if ( is_front_page() ) {
					...
					if ((!empty($slider_type)) && ($slider_type!="none") && $is_revslider) {
						...
						switch($slider_type) {
							case "img-slider":
								...
								$custom_slider = isset($meta['cws-mb-slider']) ? $meta['cws-mb-slider'] : null;
								if ( ( isset($meta['cws-mb-sb_slider_override']) ) && ( !empty($custom_slider ) ) ) {
									echo do_shortcode($custom_slider);
								} elseif (!empty($slider_shortcode)) {
					...
					if ( ( isset($meta['cws-mb-sb_slider_override']) ) && ( !empty($slider) ) ){
						...
						echo '<section class="page-title"><div class="container clearfix">';
							...
							echo ( is_archive() ? ('<h1>' . get_the_archive_title() . '</h1>') : ('<h1>' . $page_tile . '</h1>'));
			...
			<div ng-show="blocked == true"
				 class="modal fade"
				 ng-class="{'in': blocked == true}"
				 style="{{ popupStyle }}"
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							{{ blockPopupMessage }}<br />
							<button style="float:right" class="btn btn-primary m-bot10" ng-click="unBlockPopup();">Entra</button><br/><br />
						</div>
					</div>
				</div>
			</div>
