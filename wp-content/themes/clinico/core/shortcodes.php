		...
		if (!array_key_exists('error', $twt_obj)) {
			...
			if ($twt_obj) {
				...
				foreach ($twt_obj as $tweets) {
					...
					$tweet_text = preg_replace('@(https?://([-\w\.]+)+(:\d+)?(/([\w/_\.]*(\?\S+)?)?)?)@', '<a href="$1" target="_blank">$1</a>', $tweets['text']);
...
function shortcode_button($atts, $content){
	...
	$out = "<a href='$link' " . ($target ? " target='_blank'" : "" ) . " class='cws_button " . $class . "'" . ( $custom_color ? " data-bg-color='" . $button_color . "'" . " data-font-color='" . $text_color . "'" . " data-border-color='" . $border_color . "'" : "" ) . ">" . do_shortcode($content) . "</a>";
	return $out;
}
