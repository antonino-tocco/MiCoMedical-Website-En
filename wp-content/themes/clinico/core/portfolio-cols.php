<?php
	if(!empty($_POST['action'])) {
		...
		if (!empty($action[9])) {
			global $sitepress;
			$sitepress->switch_lang($action[9], true);
		}
		...
		if (!empty($filter[9])) {
			global $sitepress;
			$sitepress->switch_lang($filter[9], true);
		}
...
function render_portfolio ($cols = 2,
	...
	if ($p->have_posts()) :
		...
		endwhile;
		$content .=  $bAjax ? '' : '</div></div>'; // class=grid
		if ( $posts_per_page != '-1' ) {
	...
	switch ($cp_type) {
		...
		case 'staff':
			...
			if (!empty($cws_stored_meta)){
				...
				$out .= '<a href='. $link .'><img src="' . bfi_thumb($img[0], array('width' => 270, 'height' => 270) ) . '" alt=""></a>';
			}

			$out .= "</div><div class='team_member_info'>";
