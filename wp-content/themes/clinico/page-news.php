		...
		<main>
			<div class="col-sm-8 col-xs-12">
				<div class="col-sm-12 col-xs-12" ng-controller="NewsController">
					...
					<div class="page" ng-repeat="page in newsPages track by $index"
						ng-class="{'is-hidden': $index === newsSelectedPage}">
						<div class="row news-row" ng-repeat="news in page.newsList track by $index">
							<p><strong>{{ news.date }} </strong> - <a href="{{ news.url }}">{{ news.title }}</a></p>
						</div>
					</div>
					<div class="pagination pagination-lg">
						<a href="#"ng-repeat="page in newsPages track by $index"
						   ng-class="{'active': newsSelectedPage === $index}"
						   ng-click="newsSelectedPage = $index">{{ $index + 1 }}</a>
					</div>
				</div>
				<div class="col-sm-12 col-xs-12" ng-controller="EventsController">
					...
					<div class="page" ng-repeat="page in eventsPages track by $index"
						 ng-class="{'is-hidden': $index === eventsSelectedPage}">
						<div class="row news-row" ng-repeat="event in page.eventsList track by $index">
							<p><strong>EVENT - {{ event.date }} </strong> - <a href="{{ event.url }}">{{ event.title }}</a></p>
						</div>
					</div>
					<div class="pagination pagination-lg">
						<a href="#" ng-repeat="page in eventsPages track by $index"
						   ng-class="{'active': eventsSelectedPage === $index}"
						   ng-click="eventsSelectedPage = $index">{{ $index + 1 }}</a>
					</div>
				</div>
			</div>
